...
void delay_ms(uint16_t delay)
{
	...
	SET_BIT (RCC->APB2ENR, RCC_APB2ENR_TIM10EN ); //тактирование таймера 10
	CLEAR_REG(TIM10->CNT);
	WRITE_REG (TIM10->PSC, (uint16_t)(HAL_RCC_GetPCLK1Freq()/1000) - 1); //GetPCLK1Freq - частота шины таймера 10 						
	...
	while (!READ_BIT(TIM10->SR, TIM_SR_UIF)) {} //ожидание установки флага обновления таймера 10
