...
//		GPIO_CAN_init.GPIO_PuPd = GPIO_PuPd_UP; //подтяжка к питанию
	...
	CAN_InitStruct.CAN_ABOM = DISABLE; //Если режим включен, то при накоплении ошибок приема данных из шины, CAN автоматически будет отключен.
	...
	CAN_InitStruct.CAN_NART = ENABLE; //Режим проверки получения пакета. Если этот режим включен, то МК при передаче кадра не будет проверять подтверждение получения пакета на шине. Если выключен - то, при передаче кадра, МК будет слушать шину на предмет получения подтверждения от всех устройства о том, что пакет получен и, если не получен хотя бы 1 устройством, то будет пытаться отправить повторно до тех пор, пока все устройства не подтвердят получение пакета.
	CAN_InitStruct.CAN_RFLM = DISABLE; //Режим блокировки Receive FIFO. 0 - при переполнении RX_FIFO прием не прерывается , каждый новый пакет затирает предыдущий
	CAN_InitStruct.CAN_TXFP = DISABLE; //Порядок отправки сообщений в шину. Если ENABLE, то сообщения отправляются в хронологическом порядке FIFO. Если DISABLE, то пакеты передаются в зависимости от приоритета ID пакета (с более высоким приоритетом будет отправлен раньше).

	/*-------------------------------baudrate = 250 кБит/с при 20MHz----------------------------*/
...
//	CAN_InitStruct.CAN_Mode = CAN_Mode_Silent_LoopBack; //В данном режиме все пакеты будут полностью крутится внутри контроллера без выхода в общую шину.
	...
	CAN_InitStruct.CAN_BS1 = CAN_BS1_4tq; //BS1 - определяет местоположение точки захвата (Sample Point). Он включает в себя Prop_Seg и PHASE_SEG1 стандарта CAN. Его продолжительность программируется от 1 до 16 квантов
	CAN_InitStruct.CAN_BS2 = CAN_BS2_4tq; //BS2 - определяет местоположение точки передачи. Он представляет собой PHASE_SEG2 стандарта CAN. Его продолжительность программируется от 1 до 8 квантов времени
	CAN_InitStruct.CAN_Prescaler = 160; //предделитель САN
...
/*-----------------------------------отправка подтверждения получения тестовой команды в шину CAN1--------------------------------------*/
	...
	{
		transmit_mailbox = CAN_Transmit(CAN1, TxMessage); //передадим сообщение
		uwCounter = 0;
		while ((status_Tx = (CAN_TransmitStatus(CAN1, transmit_mailbox))  !=  CAN_TxStatus_Ok) && (uwCounter  !=  0xFFFF))
			{
			uwCounter++; //ждём установки флага CAN_TSR_RQCPх, CAN_TSR_TXOKx, CAN_TSR_TMEx или выходим по тайм-ауту
...
void init_CAN2 ()
{
	...
	CAN_InitStruct.CAN_TXFP = DISABLE; //Порядок отправки сообщений в шину. Если параметр включен, то сообщения отправляются в хронологическом порядке: FIFO. Если же выключен, то пакеты передаются в зависимости от приоритета ID пакета (пакет с более высоким приоритетом будет отправлен раньше).
...
//	CAN_InitStruct.CAN_Mode = CAN_Mode_Silent_LoopBack; //В данном режиме все пакеты будут полностью крутится внутри контроллера без выхода в общую шину.
	...
	CAN_InitStruct.CAN_BS1 = CAN_BS1_4tq; //BS1 - определяет местоположение точки захвата (Sample Point). Он включает в себя Prop_Seg и PHASE_SEG1 стандарта CAN. Его продолжительность программируется от 1 до 16 квантов
	CAN_InitStruct.CAN_BS2 = CAN_BS2_4tq; //BS2 - определяет местоположение точки передачи. Он представляет собой PHASE_SEG2 стандарта CAN. Его продолжительность программируется от 1 до 8 квантов времени
	CAN_InitStruct.CAN_Prescaler = 16; //предделитель САN
...
/*	NVIC_InitTypeDef NVIC_InitStruct2;
	...
	CAN_ITConfig(CAN2, CAN_IT_EWG | CAN_IT_EPV | CAN_IT_LEC | CAN_IT_ERR, ENABLE);
}
