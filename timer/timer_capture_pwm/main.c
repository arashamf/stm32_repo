...
volatile unsigned int Period = 0; // измереный период импульса
volatile unsigned int Width = 0; // измереная длительность импульса
...
if(Period_Flag == 0) // флаг для измерения периода - первый импульс
	...
	TIM3->CCER |= TIM_CCER_CC2P ; //переключаем захват на спад импульса
	}
else
	if(Period_Flag == 1) // флаг для измерения периода - второй импульс
		{
		Period_Flag = 2;
		Width = TIM3->CCR2 - capture1; // измеренная длина импульса
		TIM3->CCER &= ~TIM_CCER_CC2P ; //переключаем захват на фронт импульса
		}
	else
			{
...
while(1)
{
// выводим значение периода и длительности импульса
sprintf(msg, "Pulse Width = %d mks, Period = %d mks\r\n", Width/16, (Period/16 - 1)); // 1 такт таймера 1/16 мкс
UART3_TX_string(msg);
