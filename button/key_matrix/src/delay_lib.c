...
void ini_TIMER4 ()
{
	...
	TIM4_ini.TIM_Prescaler = (2*RCC_Clocks.PCLK1_Frequency/1000000) - 1; //предделитель
	TIM4_ini.TIM_CounterMode = TIM_CounterMode_Up; //счёт на увеличение
	TIM4_ini.TIM_Period = 1000 - 1; //значение перезагрузки
	...
	NVIC_EnableIRQ(TIM4_IRQn); //разрешим прерывания от таймера 4 в контроллере прерывания 1 раз в 1мс
