...
void LEDS_ini ()
{
	...
	GPIO_InitTypeDef 	GPIO_Init_button_scan;
	GPIO_Init_button_scan.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3;
	GPIO_Init_button_scan.GPIO_Mode = GPIO_Mode_OUT; //порт E 0, 1, 2, 3 пин на обычный выход
	GPIO_Init_button_scan.GPIO_Speed = GPIO_High_Speed; //скорость порта самая низкая
	GPIO_Init_button_scan.GPIO_OType = GPIO_OType_PP; //выход с подтяжкой (push-pull)
	GPIO_Init_button_scan.GPIO_PuPd = GPIO_PuPd_NOPULL; //вход без подтяжки

	GPIO_Init (GPIOE, &GPIO_Init_button_scan);

	// настройка линий данных на вход с подтягивающими резистором к '1'
	CLEAR_BIT(GPIOE->MODER, GPIO_MODER_MODER4); //PE4 - на вход
	CLEAR_BIT(GPIOE->MODER, GPIO_MODER_MODER5); //PE5 - на вход
	CLEAR_BIT(GPIOE->MODER, GPIO_MODER_MODER6); //PE6 - на вход
	SET_BIT(GPIOE->PUPDR, GPIO_PUPDR_PUPDR4_0); // включим подтягивающий резистор к "1"
	SET_BIT(GPIOE->PUPDR, GPIO_PUPDR_PUPDR5_0); // включим подтягивающий резистор к "1"
	SET_BIT(GPIOE->PUPDR, GPIO_PUPDR_PUPDR6_0); // включим подтягивающий резистор к "1"
}
