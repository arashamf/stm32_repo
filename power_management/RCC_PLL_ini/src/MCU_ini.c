...
void DMA1_ini ()
	{
	 ...
	 ini_DMA1_UART2.DMA_Channel = DMA_Channel_4; //номер канала DMA
	 ini_DMA1_UART2.DMA_PeripheralBaseAddr = (uint32_t) &(USART2->DR); //указатель на регистр памяти перифирии: буффер USART2
	 ini_DMA1_UART2.DMA_Memory0BaseAddr = (uint32_t) message; //указатель на массив передаваемых данных; указатель на массив есть название массива
	 ini_DMA1_UART2.DMA_DIR = DMA_DIR_MemoryToPeripheral; //направление копирования, из памяти в периферию
	 ini_DMA1_UART2.DMA_BufferSize = sizeof (message); //размер буффера DMA1
