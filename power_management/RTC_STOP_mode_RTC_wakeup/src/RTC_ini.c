...
//	RCC_BackupResetCmd(DISABLE); //сбросим бит RCC_BDCR_BDRST
	...
	RTC_clock24_ini.RTC_AsynchPrediv = 0x7F; //!асинхронный делитель не может быть равен 0, 128-1=127 (стр.9 AN3371)
	RTC_clock24_ini.RTC_SynchPrediv = 0xF9;  //синхронный делитель 250-1=249
...
uint8_t UART_RTC_set_time (char *buffer_RX)
{
...
uint8_t UART_RTC_set_date (char * buffer_RX)
{


	uint8_t flag_error = 1; //флаг ошибки. 0: ошибка данных, 1: данные корректны
	uint8_t day = 0, month = 0,  year = 0;
	if ((day = (buffer_RX [0]-0x30)*10 + (buffer_RX [1]-0x30)) > 31) //вычтем 0x30, чтобы перевести знаки в цифры и проверим корректность полученных значений даты
		flag_error = 0;													//если данные некорректны, вернём флаг ошибки = 0
	if ((month = (buffer_RX [2]-0x30)*10 + (buffer_RX [3]-0x30)) > 12)
		flag_error = 0;
	if ((year = (buffer_RX [4]-0x30)*10 + (buffer_RX [5]-0x30)) > 99)
		flag_error = 0;
	if (flag_error) //если флаг ошибки = 1, т.е. данные для установки времени корректны
		{
		RTC_EnterInitMode(); //входим в режим инициализации часов
...
//---------------------------ф-ия инициализации будильника А RTC с выходом сигнала тревоги на пин PC 13 в SPL-------------------------------------//
void ALARM_ini_IRQ_SPL (uint8_t seconds)
{
	...
	RTC_ALARM_A_ini.RTC_AlarmDateWeekDay = RTC_Weekday_Monday;  //выбор конкректного дня недели. В данном случае неважно
	RTC_SetAlarm (RTC_Format_BIN, RTC_Alarm_A, &RTC_ALARM_A_ini); //установим настройки АЛАРМ А, BCD обязательно

	//----------------------инициализация пина PC13-------------------//
	RCC_AHB1PeriphClockCmd (RCC_AHB1Periph_GPIOC, ENABLE); //затактируем порт С

	GPIO_InitTypeDef 	GPIO_Init_AF_RTC; //настроим выход Аларма PC13
	GPIO_Init_AF_RTC.GPIO_Pin = GPIO_Pin_13 ; //PC13
	GPIO_Init_AF_RTC.GPIO_Mode =  GPIO_Mode_OUT; //порт C пин 13 в альт. режим
	GPIO_Init_AF_RTC.GPIO_Speed = GPIO_Fast_Speed; //скорость порта высокая
	GPIO_Init_AF_RTC.GPIO_OType = GPIO_OType_PP; //выход с подтяжкой (push-pull)
	GPIO_Init_AF_RTC.GPIO_PuPd =  GPIO_PuPd_NOPULL; //вход без подтяжки (неважно)
	GPIO_Init (GPIOC, &GPIO_Init_AF_RTC);

	GPIO_PinAFConfig (GPIOC, (GPIO_PinSource13), GPIO_AF_RTC_50Hz); //установим альт. функцию RTC

	RTC_OutputConfig(RTC_Output_AlarmA, RTC_OutputPolarity_High); //на выход (PC13) RTC будет подаваться сигнал с Аларма А, пр этом сигнал на выходе устанавливается в 1
	RTC_OutputTypeConfig(RTC_OutputType_PushPull); //выход RTC в режиме подтяжки

	//----------------------настройка прерывания от аларма А------------------------//
	EXTI_InitTypeDef RTC_ALARMA_EXTI17_ini; //структура для настройки линии прерывания EXTI №17
	RTC_ALARMA_EXTI17_ini.EXTI_Line = EXTI_Line17; //включим линию EXTI №17 для Аларма
	RTC_ALARMA_EXTI17_ini.EXTI_Mode = EXTI_Mode_Interrupt; //режим прерываний на линии EXTI №17 (можно ещё выбрать event mode)
	RTC_ALARMA_EXTI17_ini.EXTI_Trigger = EXTI_Trigger_Rising; //срабатывание линии 17 по нарастающему фронту
	RTC_ALARMA_EXTI17_ini.EXTI_LineCmd = ENABLE; //включим линию 17
	EXTI_Init(&RTC_ALARMA_EXTI17_ini);

	RTC_ITConfig(RTC_IT_ALRA, ENABLE);//включим прерывание от аларма А
	NVIC_EnableIRQ (RTC_Alarm_IRQn); //включим глобальное прерывание от аларма
...
//---------------------------ф-ия инициализации таймера wakeup в SPL-------------------------------------//
void WAKEUP_ini_SPL ()
{
	RTC_WakeUpCmd (DISABLE); //отключим таймер пробуждения
	while (!RTC_GetFlagStatus(RTC_FLAG_WUTWF)); //опрос флага WUTWF (разрешения обновления данных таймера wakeup)
	RTC_WakeUpClockConfig (RTC_WakeUpClock_RTCCLK_Div16); //значение предделителя таймера
	RTC_SetWakeUpCounter(10000-1); //значение перезагрузки. Срабатыванийе таймера Counter/F_LSI/Div=10000/(32000/16)=5с

	//----------------------настройка прерывания от таймера wakeup------------------------//
	EXTI_InitTypeDef RTC_WAKEUP_EXTI22_ini; //структура для настройки линии прерывания EXTI №22
	RTC_WAKEUP_EXTI22_ini.EXTI_Line = EXTI_Line22; //включим линию EXTI №22 для Аларма
	RTC_WAKEUP_EXTI22_ini.EXTI_Mode = EXTI_Mode_Interrupt; //режим прерываний на линии EXTI №22 (можно ещё выбрать event mode)
	RTC_WAKEUP_EXTI22_ini.EXTI_Trigger = EXTI_Trigger_Rising; //срабатывание линии 22 по нарастающему фронту
	RTC_WAKEUP_EXTI22_ini.EXTI_LineCmd = ENABLE; //включим линию 22
	EXTI_Init(&RTC_WAKEUP_EXTI22_ini);

	RTC_ITConfig(RTC_IT_WUT, ENABLE);//включим прерывание от таймера wakeup
	NVIC_EnableIRQ (RTC_WKUP_IRQn); //включим глобальное прерывание от аларма

	RTC_WakeUpCmd (ENABLE); //включим таймер пробуждения
}
