...
void MX_TIM1_Init(void)
{
  ...
  */
  ...
  LL_TIM_IC_SetPrescaler(TIM1, LL_TIM_CHANNEL_CH1, LL_TIM_ICPSC_DIV2);
  ...
  LL_TIM_IC_SetPrescaler(TIM1, LL_TIM_CHANNEL_CH2, LL_TIM_ICPSC_DIV2);
...
void tim_delay_init (void)
{
  ...
  LL_APB1_GRP1_EnableClock(LL_APB1_GRP1_PERIPH_TIM3);   // Peripheral clock enable 

  TIM_InitStruct.Prescaler = (uint16_t)((CPU_CLOCK_VALUE/1000000)-1); //предделитель (72-1)
  TIM_InitStruct.CounterMode = LL_TIM_COUNTERMODE_UP;
...
{
  LL_TIM_SetAutoReload(TIM3, delay); //
	LL_TIM_ClearFlag_UPDATE(TIM3); //сброс флага обновления таймера
	LL_TIM_SetCounter(TIM3, 0); //сброс счётного регистра
	LL_TIM_EnableCounter(TIM3); //включение таймера
	while (LL_TIM_IsActiveFlag_UPDATE(TIM3) == 0) {} //ожидание установки флага обновления таймера 
	LL_TIM_DisableCounter(TIM3); //выключение таймера		
}
/* USER CODE END 1 */
