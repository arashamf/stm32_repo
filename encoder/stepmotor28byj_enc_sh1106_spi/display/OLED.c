...
void OLED_SendCommand(uint8_t command)
{
	LCD_CS(0);
	LCD_DC(0);
	HAL_SPI_Transmit(hspi, &command, 1, 0x1);
	LCD_CS(1);
}
...
uint8_t OLED_init(void)
{
  ...
  delay_us (3);	
	...
	OLED_SendCommand(OLED_DISPLAY_OFF); // Display Off
	...
	OLED_SendCommand(OLED_CHARGE_PUMP); //set pump voltage value
	OLED_SendCommand(0x14);
	
	OLED_SendCommand(OLED_MEMORY_ADDR_MODE);
	OLED_SendCommand(0x00);
	
	OLED_SendCommand(OLED_SET_SEGMENT_REMAP | 0x0); //0x1 - left rotation, 0x0 - right rotation
	...
	OLED_SendCommand(OLED_DISPLAY_ON);  // Display On
...
{
	if (mode) 
		OLED_ClearBuffer(0xFF); //чёрные пиксели
	else      
		OLED_ClearBuffer(0x00); //белые пиксели
}
...
{
	if (inverted)	
		OLED_SendCommand(OLED_INVERT_DISPLAY); //инвертирование цвета - черные буквы на белом
	else			
		OLED_SendCommand(OLED_NORMAL_DISPLAY);
}
...
void OLED_DrawCharSetMode(char c, uint8_t ix, uint8_t iy, uint8_t mode)
{
	...
	for (uint8_t x = 0; x < symbol[0]; x++)
	{
		for (uint8_t y = 0; y < Font.height; y++)
		{
			...
			{	
				OLED_DrawPixelStatus(ix + x, iy + y, (mode) ? 1 : 0); //возможность выбора режима
			}
			else
			{	
...
void OLED_DrawChar(char c, uint8_t ix, uint8_t iy)
{
	...
	for (uint8_t x = 0; x < symbol[0]; x++)
	{
		for (uint8_t y = 0; y < Font.height; y++)
		{
			...
			if (symbol[1 + y / 8 + x * (Font.numchars - 1) / Font.width] & (0x01 << (y % 8)))
			{	OLED_DrawPixelStatus(ix + x, iy + y, 1); } //без инверсии
			else
