...
void UART2_ini ()
	...
	ini_USART2.USART_Mode = USART_Mode_Rx | USART_Mode_Tx;  //включит приём/передачу по USART2
...
Настраивает, когда DMAy Streamx отключен, режим двойного буфера и текущую цель памяти.
DMAy_Streamx: где y может быть 1 или 2 для выбора DMA, а x может быть от 0 до 7 для выбора потока DMA.
Memory1BaseAddr: базовый адрес второго буфера (Memory 1)
DMA_CurrentMemory: указывает, какая память будет первым буфером для транзакций, когда поток будет включен.
Этот параметр может иметь одно из следующих значений: DMA_Memory_0: память 0-это текущий буфер. DMA_Memory_1: память 1-это текущий буфер.

void DMA_MemoryTargetConfig(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t MemoryBaseAddr, uint32_t DMA_MemoryTarget);
Настраивает адрес памяти для следующей передачи буфера в режиме двойного буфера (для динамического использования).
Эта функция может быть вызвана, когда поток DMA включен и когда передача продолжается.
DMAy_Streamx: где y может быть 1 или 2 для выбора DMA, а x может быть от 0 до 7 для выбора потока DMA.
MemoryBaseAddr: базовый адрес целевого буфера памяти
DMA_MemoryTarget: следующая цель памяти, которая будет использоваться.
Этот параметр может иметь одно из следующих значений: DMA_Memory_0: для использования адреса памяти 0. DMA_Memory_1: для использования адреса памяти 1
*/
