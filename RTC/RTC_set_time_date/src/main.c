//-----------------------------------инициализация RTC c будильником--------------------------------------//
#include "stdio.h"
#include "stm32f4xx.h"
#include "delay_lib.h"
#include "MCU_ini.h"
...
extern RTC_DateTypeDef RTC_set_date; //инициализируем шаблон структуры установки даты
...
//---------------------------ф-ия получения и проверки настроек даты RTC-------------------------------------//
void check_and_set_date ()
{
	uint8_t count = 0; //значение счётчика
	while (!receive_flag) //цикл опроса пользователя, до тех пока пока не приняты корректные данные
	{
		if (((count++) % 4) == 0) //выводим сообщения при count = 4, т.е. раз в 2 с
			{
			UART2_PutRAMString ("please, set date\r\n");
			UART2_PutRAMString ("format: ddmmyy, example: 040320\r\n");
			}
		GPIO_WriteBit(GPIOD, (GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15), Bit_SET); //помигаем светодиодами
		delay_ms (DELAY/4); // задержка 1/4 c
		GPIO_WriteBit(GPIOD, (GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15), Bit_RESET);
		...
		if (receive_flag) //если флаг принятия данных по UART установлен
			{
			if (UART_RTC_set_date (recieve_msg)) //если функция установки даты не вернула ошибку
				{
				receive_flag = 0; //обнулим флаг принятия данных
				break;			 //выйдем из цикла опроса
				}
				...
				{
				UART2_PutRAMString ("data date error!\r\n"); //предупредим об ошибке и продолжим цикл опроса
				}
			receive_flag = 0; //обнулим флаг принятия данных
			}
		if (count == 4) //если счётчик равен 4
...
int main(void)
{
	...
	UART2_PutRAMString ("set date and time\r\n");
	check_and_set_time (); //висим в этой функции, пока не будут получены корректные данные времени
	check_and_set_date (); //висим в этой функции, пока не будут получены корректные данные латы
	NVIC_DisableIRQ(USART2_IRQn); //отключим прерывание от USART2
	ALARM_ini (SIGNAL); //установим будильник на каждую 45 секунду
	...
	while (1)
	{
		...
		UART2_PutRAMString (msg); // выводим строку по UART
		if(READ_BIT (RTC->ISR, RTC_ISR_ALRAF)) //Этот флаг устанавливается АО, когда регистры времени/даты (RTC_TR и TC_DR) соответствуют регистру ALRMAR
			{
			CLEAR_BIT (RTC->ISR, RTC_ISR_ALRAF); // сбросим флаг будильника A
