/**
  ...
  ******************************************************************************
*/
//-----------------------------------инициализация RTC c прерываниями от 2 будильников--------------------------------------//
#include "stdio.h"
#include "stm32f4xx.h"
#include "delay_lib.h"
#include "MCU_ini.h"
...
#define SIGNAL_B 25 //время срабатывания аларма B
...
uint8_t flag_ALM_B = 0; //флаг срабатывания прерывания от аларма B
...
void RTC_Alarm_IRQHandler(void)
{
	...
	if (READ_BIT (RTC->ISR, RTC_ISR_ALRBF))
		{
		CLEAR_BIT (RTC->ISR, RTC_ISR_ALRBF); // сбросим флаг будильника A (сбрасывается путем записи "0")
		SET_BIT (EXTI->PR, EXTI_PR_PR17); // сбросим флаг линии внешнего прерывания EXTI17 (сбрасывается путем записи "1", а не "0")
		flag_ALM_B = 1; // установить флаг срабатывания прерывания будильника А
		}
}
...
int main(void)
{
	...
	ALARMB_ini_IRQ (SIGNAL_B); //установим будильник A на каждую 25 секунду
	...
	while (1)
	{
		...
		if(flag_ALM_B)
			{
			UART2_PutRAMString("ALARM B!!!\r\n"); // выводим строку по UART
			flag_ALM_B = 0; //обнулим флаг
			}
