...
//	RCC_DeInit(); //сбросим настройки RCC
	...
	RCC->CR &= (uint32_t)0xEAF6FFFF; //Reset HSEON, CSSON, PLLON, PLLI2S and PLLSAI bits
	...
	RCC->PLLCFGR |= RCC_PLLM | (RCC_PLLN << 6) | (((RCC_PLLP >> 1) -1) << 16) ;//сдвигаем биты RCC_PLLN и RCC_PLLP, в данном случае: 8МГц*192/8/2=96МГц
...
uint8_t PLL_ini_SPL (uint8_t CPU_WS, uint8_t RCC_PLLM, uint32_t RCC_PLLN, uint32_t RCC_PLLP, uint8_t RCC_PLLQ)
{
	...
	RCC_PCLK1Config (RCC_HCLK_Div2); // установим делитель шины APB1 - 4 (42 МГц макс. для APB1)
	RCC_PCLK2Config (RCC_HCLK_Div2); // установим делитель шины APB2 - 4 (84 МГц макс. для APB2)
