...
void PLL_ini (uint8_t CPU_WS, uint8_t RCC_PLLM, uint32_t RCC_PLLN, uint32_t RCC_PLLP)
{
	...
	SET_BIT (RCC->CFGR, RCC_CFGR_PPRE1_2 | RCC_CFGR_PPRE1_0);// установим делитель шины APB1 - 4 (SYSCLK/ (42 МГц макс. для шины APB1) = 4)
	SET_BIT (RCC->CFGR, RCC_CFGR_PPRE2_2 | RCC_CFGR_PPRE2_0);// установим делитель шины APB2 - 4 (SYSCLK/ 42 (84 МГц макс. для APB2) = 4)
