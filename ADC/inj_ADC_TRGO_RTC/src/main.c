// Инжектированные каналы. Запуск по триггеру (таймер). Прерывания.
// Измерение внутренней температуры, напряжения батареи RTC и внутреннего опорного напряжения
#define stm32f4xx
...
void ADC_IRQHandler(void)
{
	if(ADC1->SR & ADC_SR_JEOC) // если сработало прерывание от ADC1_JEOC
	{
		...
		sprintf(msg,("ADC_Temp = %f C, ADC_Vref = %f, ADC_Vbat = %f V\r\n"), (((Vref*ADC1->JDR1/4096)-V25)/Avg_Slope+25.0), (Vref*ADC1->JDR2/4096),
		(Vref*ADC1->JDR3/4096)); // выводим значение АЦП в строку
//		sprintf(msg,("ADC_Temp = %f volt\r\n"), (Vref*ADC1->JDR1/4096));
